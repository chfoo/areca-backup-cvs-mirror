...
public abstract class AbstractProcessor implements Processor {
    ...
    public boolean shallRun(ProcessContext context) {
    	return
    		...
    		|| (runScheme == Processor.RUN_SCHEME_FAILURE && context.getReport().hasError())
    		|| (runScheme == Processor.RUN_SCHEME_SUCCESS && (! context.getReport().hasError()));
    }
	...
	public void run(ProcessContext context) throws ApplicationException {
        try {
            ...
            if (shallRun(context)) {
            	...
            	Logger.defaultLogger().info("The processor will not be run (rule = \"" + getSchemeAsString() + "\" and state = \"" + (context.getReport().hasError() ? "Failure":"Success") + "\")");
            }
